...
std::uint32_t simulationSeed(const edm::ParameterSet &config)
{
  ...
  if (seed == 0)
  {
    throw cms::Exception("Configuration") << "G4SimProducer randomSeed must be positive";
  }
  return seed;
}
...
public:
  ...
  static std::unique_ptr<c4h::G4MasterInterface> initializeGlobalCache(const edm::ParameterSet &iConfig);

  static std::shared_ptr<int> globalBeginRun(const edm::Run &iRun, const edm::EventSetup &iSetup,
...
// INLINE DEFINITIONS
//---------------------------------------------------------------------------//
std::unique_ptr<c4h::G4MasterInterface> G4SimProducer::initializeGlobalCache(const edm::ParameterSet &iConfig)
{
  return std::make_unique<c4h::G4MasterInterface>(iConfig);
}

G4SimProducer::G4SimProducer(const edm::ParameterSet &p, const c4h::G4MasterInterface *masterThread)
    : m_handoff{p.getUntrackedParameter<int>("workerThreadStackSize", 10 * 1024 * 1024)}, m_masterThread(masterThread),
      randomSeed_(simulationSeed(p)), mcToken_(consumes(p.getParameter<edm::InputTag>("generator"))),
      trackerHitsToken_(produces<edm4hep::SimTrackerHitCollection>("SimTrackerHits")),
      calorimeterHitsToken_(produces<edm4hep::SimCalorimeterHitCollection>("SimCalorimeterHits"))
{
...
// MEMBER FUNCTIONS
//---------------------------------------------------------------------------//
std::shared_ptr<int> G4SimProducer::globalBeginRun(const edm::Run &iRun, const edm::EventSetup &iSetup,
                                                   const c4h::G4MasterInterface *masterThread)
{
  if (masterThread)
  {
...
void G4SimProducer::globalEndRun(const edm::Run &iRun, const edm::EventSetup &iSetup, const RunContext *iContext)
{
...
void G4SimProducer::produce(edm::Event &e, const edm::EventSetup &es)
{
  ...
  m_handoff.runAndWait(
      [this, &e, &products, token]()
      {
        ...
        m_workerInterface->produce(g4evt.get(), c4h::geantEventSeed(randomSeed_, e.id().run(), e.id().event()));
