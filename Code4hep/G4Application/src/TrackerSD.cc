 ...
 */
TrackerSD::TrackerSD(G4String name) : G4VSensitiveDetector(name), hcid_(-1), collection_(nullptr)
{
...
{
  collection_ = new TrackerHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool TrackerSD::ProcessHits(G4Step *step, G4TouchableHistory *)
{
  ...
  const auto key = (static_cast<std::uint64_t>(trackID) << 32U) | static_cast<std::uint64_t>(id);
  ...
  if (const auto found = hitByTrackAndCell_.find(key); found != hitByTrackAndCell_.end())
  {
    found->second->add(step->GetTotalEnergyDeposit(), step->GetStepLength());
    return true;
  }
  auto *hit = new TrackerHit(id, trackID, step->GetTotalEnergyDeposit(), time, step->GetStepLength(), pos, momentum);
