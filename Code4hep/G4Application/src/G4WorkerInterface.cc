...
int get_new_thread_index() { return thread_counter.fetch_add(1, std::memory_order_relaxed); }
} // namespace

namespace c4h
{
G4WorkerInterface::G4WorkerInterface() : threadIndex_{(get_new_thread_index())}
{
  // Assign Geant4 thread id for thread-local bookkeeping
  G4Threading::G4SetThreadId(threadIndex_);
}

G4WorkerInterface::~G4WorkerInterface() { workerRM_.reset(); }
...
void G4WorkerInterface::initializeG4(G4MTRunManager *masterRM)
{
  ...
  workerRM_.reset(masterRM->GetUserWorkerThreadInitialization()->CreateWorkerRunManager());
  ...
  if (workerRM_->GetWorkerRunManagerKernel())
  {
    ...
    G4UserRunAction *userRunAction = const_cast<G4UserRunAction *>(workerRM_->GetUserRunAction());
  ...
  {
    throw cms::Exception("Configuration") << "Illegal G4WorkerRunManagerKernel state";
  }
...
G4Event *G4WorkerInterface::produce(G4Event *g4evt, long randomSeed)
{
  if (g4evt == nullptr)
  {
  ...
  if (!kernel)
  {
    throw cms::Exception("Configuration") << "WorkerRunManagerKernel not available";
  }
